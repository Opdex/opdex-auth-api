...
public class AuthorizeRequestQueryValidator : AbstractValidator<AuthorizeRequestQuery>
{
    ...
    {
        RuleFor(request => request.ResponseType).MustBeValidEnumValue()
            .WithMessage("Response type must be code or sid");
        
        When(request => request.ResponseType == ResponseType.Code, () =>
        {
